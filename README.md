...
La API está organizada en **cuatro capas**, cada una con una única responsabilidad:
...
- **Service**: aplica las reglas de negocio (por ejemplo: "no se puede agregar a una reserva un servicio que no existe", la regla de
  ...
  dominio como `{ error: 'SERVICE_NOT_FOUND' }`). Un service puede **componer** a otro (`bookings.service` usa `services.service` para
  validar servicios).
